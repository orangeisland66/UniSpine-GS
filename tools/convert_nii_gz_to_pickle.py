...
def config_parser():
    ...
    parser.add_argument("--input_nii_gz", default=None, type=str,
                        help="Input .nii or .nii.gz file")
    parser.add_argument("--output_pickle", default=None, type=str,
                        help="Output pickle path")
    parser.add_argument("--config", default=str(DEFAULT_CONFIG), type=str,
                        help="Cone-beam geometry YAML")
    parser.add_argument("--gpu_id", default=None, type=str,
                        help="CUDA_VISIBLE_DEVICES value used by TIGRE")
    parser.add_argument("--vis_dir", default=None, type=str,
                        help="Optional directory for conversion preview images")
    parser.add_argument("--ctName", default="head", type=str,
                        help="Name of CT")
    parser.add_argument("--inputName", default="ct.nii.gz", type=str,
                        help="Input file name to replace the default 'ct.nii.gz'")
...
def main():
    ...
    if args.input_nii_gz is not None or args.output_pickle is not None:
        if args.input_nii_gz is None or args.output_pickle is None:
            parser.error("--input_nii_gz and --output_pickle must be provided together")
        generator(
            str(Path(args.input_nii_gz).expanduser().resolve()),
            args.config,
            str(Path(args.output_pickle).expanduser().resolve()),
            dataFolder="local",
            dataType="local",
            show=False,
    ...
    inputFileName = args.inputName
    outputName = args.outputName
    outputFolder = args.outputFolder
    
    if osp.isabs(inputFileName):
        matPath = inputFileName
    else:
        matPath = f"./dataGenerator/{dataFolder}/{dataType}/{inputFileName}"  # Update file extension
        
    configPath = f"./dataGenerator/{dataFolder}/{dataType}/config.yml"
    outputPath = osp.join(outputFolder, f"{outputName}.pickle")
    generator(matPath, configPath, outputPath, dataFolder, dataType, True, vis_dir=args.vis_dir)
...
def loadImage(filename, nVoxels=None, convert=True, rescale_slope=1.0, rescale_intercept=0.0, normalize=True):
    """
    Load CT image from .nii.gz file.
    """
    if nVoxels is None:
        nVoxels = np.array((256, 256, 256))
    ...
    # Check if the image data has four dimensions (time series)
    if img_data.ndim == 4:
        # Assume the first dimension is time and take the first time point
        img_data = img_data[0, :, :, :]

    # Convert from Hounsfield units to attenuation if needed
    if convert:
        print("Convert from HU to attenuation")
        img_data = convert_to_attenuation(img_data, rescale_slope, rescale_intercept)

    # Resize the image if necessary
    img_shape = img_data.shape
    zoom_factors = [nVoxels[i] / img_shape[i] for i in range(3)]
    if any(zoom_factor != 1.0 for zoom_factor in zoom_factors):
        print(f"Resize ct image from {img_shape} to {nVoxels}")
        img_data = scipy.ndimage.interpolation.zoom(img_data, zoom_factors, order=3, prefilter=False)

    # Normalize the image if needed
    img_max = np.max(img_data)
    img_min = np.min(img_data)
    if normalize and img_min != 0 and img_max != 1:
        print("Normalize range to [0, 1]")
...
def save_preview_images(vis_dir, img, train_projections, val_projections):
    if vis_dir is None:
        return

    vis_root = Path(vis_dir).expanduser().resolve()
    ct_dir = vis_root / "ct"
    train_dir = vis_root / "train_proj"
    val_dir = vis_root / "val_proj"
    ct_dir.mkdir(parents=True, exist_ok=True)
    train_dir.mkdir(parents=True, exist_ok=True)
    val_dir.mkdir(parents=True, exist_ok=True)

    show_step = 5
    show_num = train_projections.shape[0] // show_step
    show_image_train_ct = img[..., ::show_step]
    show_dir_train_proj = train_projections[::show_step, ...]
    show_dir_vali_proj = val_projections[::show_step, ...]

    for i in range(show_num):
        if i < show_image_train_ct.shape[-1]:
            iio.imwrite(str(ct_dir / f"CT_{i}.png"), _to_uint8(show_image_train_ct[..., i]))
        iio.imwrite(str(train_dir / f"projs_{i}.png"), _to_uint8(show_dir_train_proj[i, ...]))
        iio.imwrite(str(val_dir / f"projs_{i}.png"), _to_uint8(show_dir_vali_proj[i, ...]))

    print(f"Saved visualization to {vis_root}")


def generator(matPath, configPath, outputPath, dataFolder, dataType, show=False, vis_dir=None):
    """
    ...
    # STRICT STRATEGY: Midpoints
    # This guarantees that every validation view is exactly in the middle of two training views.
    # This maximizes the distance to the training set and eliminates randomness/contamination.
    
    train_angles = data["train"]["angles"]
    if len(train_angles) > 1:
        interval = train_angles[1] - train_angles[0]
    else:
        # Fallback for single view (unlikely)
        interval = (data["totalAngle"] / 180 * np.pi) / data["numTrain"]

    # Calculate midpoints
    # We take the training angles and shift them by half the interval
    val_angles_mid = train_angles + (interval / 2)
    
    # Handle case where numVal != numTrain
    if data["numVal"] <= len(val_angles_mid):
        data["val"] = {"angles": val_angles_mid[:data["numVal"]]}
    else:
        # If we need more validation views than training intervals, we cycle or extend
        # For standard usage (50 train, 50 val), this branch is not taken.
        print(f"Warning: numVal ({data['numVal']}) > numTrain ({len(train_angles)}). Extending sequence.")
        extra_needed = data["numVal"] - len(val_angles_mid)
        # Simple extension strategy: add small offsets to existing midpoints (not ideal but safe)
        extra_angles = val_angles_mid[:extra_needed] + (interval / 4)
        data["val"] = {"angles": np.concatenate([val_angles_mid, extra_angles])}
    ...
    save_preview_images(vis_dir, img, data["train"]["projections"], data["val"]["projections"])
